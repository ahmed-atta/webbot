...
function split_string($string, $delineator, $desired, $type){
	...
	$position = strpos($lc_str, $marker);
    if($position){
		# Return text BEFORE the delineator
		if($desired == BEFORE) {
			// Return text ESCL/INCL of the delineator
			$split_here = ($type == EXCL)? $position: $position + strlen($marker);  
			$parsed_string = substr($string, 0, $split_here);
		}
		# Return text AFTER the delineator
		else { 
			// Return text ESCL/INCL of the delineator
			$split_here = ($type == EXCL)?  $position + strlen($marker) : $position;  
...
/***********************************************************************
queryXPath($html,$query)                                                
-------------------------------------------------------------           
DESCRIPTION:                                                            
        Returns  HTML data by using xpath query               
INPUT:                                                                  
        $string     raw HTML  
		$string     xpath query 		
...
OUTPUT:                                                                 
        Returns a string of cleaned-up HTML                             
***********************************************************************/
 function queryXPath($html,$xpath_query) 
{
	    //add this line
	   ...
	   // $xml = simplexml_import_dom($dom);
		$contents =  $xpath->query($xpath_query);
		var_dump($contents->length);
		foreach ($contents as $entry) {
		echo "Found {$entry->previousSibling->previousSibling->nodeValue}," .
         " by {$entry->previousSibling->nodeValue}\n";
}
		//return $contents[0]->getAttribute('href');
}