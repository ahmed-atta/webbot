 ...
 <div class="tabbable" style="float:right;direction:rtl"> <!-- Only required for left/right tabs -->
  ...
  <div class="tab-content">
    ...
    <div class="tab-pane" id="tab2">
		...
		<tbody>
		<?php 
		if ($classes = $mysqli->query("SELECT * ,classes.ID AS cID,classes.name AS c_name, members.name AS m_name 
		FROM  `classes`,  `members` 
WHERE classes.member_ID = members.pid
ORDER BY classes.ID DESC ;")){
			if(mysqli_num_rows($classes) > 0 ) {  
				while($class = $classes->fetch_assoc()){
					$m_class = str_replace('#',"<br/>",$class['m_class']); 
					echo "<tr><td>".$class['cID'].
					"</td><td>".$class['c_name'].
					"</td><td>".$class['m_name'].
					"</td><td>".$class['SellType'].
					"</td><td>".$m_class."</td>
					<td><a href='?action=del&type=C&id=".$class['cID']."' class='btn btn-danger' >حذف</a></td></tr>";
				}
			}
		}
		?>
