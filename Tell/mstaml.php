...
if(isset($_POST['start']) && isset($_POST['end'])){
	...
	for($i = $start; $i<= $end; $i++ ){
		...
		//============================================//
		//echo $contents;
		@preg_match("(الجوال</td>(.*)</tr>)siU", $contents, $_mobile);
		@preg_match("(title=\"(.*)\")siU", $_mobile[1], $mob);
		if(!empty($mob[1])){
			$key = $mobs[1];
		} else {
			@preg_match("(الهاتف</td>(.*)</tr>)siU", $contents, $_phone);
			@preg_match("(title=\"(.*)\")siU", $_phone[1], $ph);
			if(isset($ph[1])){
				$key = $ph[1];
			}else {
				@preg_match("(أخرى</td>(.*)</tr>)siU", $contents, $_other);
				@preg_match("(title=\"(.*)\")siU", $_other[1], $oth);
				$key = @$oth[1];
			}
