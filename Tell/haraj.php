...
if(isset($_POST['start']) && isset($_POST['end'])){
	...
	for($i = $start; $i<= $end; $i++ ){
		...
		@preg_match("(<span class=\"label label-success\">(.*)<\/i>)siU", $contents, $matches); // 0551016333
		$match = explode(":",strip_tags($matches[0]));//print_r($match);exit;
		...
		print_r($category);
		exit;
		@preg_match('/href="http:\/\/haraj.com.sa\/city\/(.*)" /siU', $contents, $city); 
		$city[0] = $city[1];
		//print_r($city[1] );
		//exit;
		// Ad Text
		...
		preg_match("(<div class=\"cont_title\">(.*)<\/div>(.*)<div id=\"leftmenu\">)siU",$contents, $ad_text);
		 ...
		 if(!empty($match[0]) && strlen($match[0]) > 5 ){
			$number = getNumber($match[1]);
